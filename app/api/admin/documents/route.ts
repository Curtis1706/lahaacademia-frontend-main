...
export async function GET(request: NextRequest) {
  try {
    ...
    const endpoint = baseApi.includes('/api') 
      ? `${baseApi}/documents/?${params.toString()}`
      : `${baseApi}/api/documents/?${params.toString()}`
    ...
    console.log(`📋 Nombre de documents: ${data.count || data.results?.length || 0}`)
...
export async function POST(request: NextRequest) {
  try {
    ...
    const endpoint = baseApi.includes('/api') 
      ? `${baseApi}/documents/`
      : `${baseApi}/api/documents/`
    
    console.log('📝 Création d\'un document:')
    console.log(`  Endpoint: ${endpoint}`)
