...
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const endpoint = `${baseApi}/api/educational-content/${videoId}/`
    ...
    console.error('Erreur API video GET:', error)
    return NextResponse.json(
...
export async function PUT(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    console.log('📝 Modification de la vidéo:', videoId)
    console.log(`  Endpoint: ${endpoint}`)
    ...
    console.error('Erreur API video PUT:', error)
    return NextResponse.json(
      { error: 'Erreur interne du serveur' },
      { status: 500 }
    )
...
export async function DELETE(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    console.error('Erreur API video DELETE:', error)
    return NextResponse.json(
      { error: 'Erreur interne du serveur' },
      { status: 500 }
    )
